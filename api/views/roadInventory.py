...
def roadInventory_province(request):
    """
    List all RoadInventory items for links under the user's province or create a new one.
    """
...
@permission_classes([IsAuthenticated])
def road_inventory_detail(request, pk):
    """
    Retrieve, update, or delete a specific RoadInventory instance.
    """
    logged_in_user = request.user
    ...
    road_inventory = get_object_or_404(RoadInventory, pk=pk)

    if road_inventory.linkId.province != logged_in_user.province:
        return Response({'detail': 'Unauthorized access.'}, status=status.HTTP_403_FORBIDDEN)

    if request.method == 'GET':
        serializer = RoadInventorySerializer(road_inventory)
