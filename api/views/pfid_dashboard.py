...
def pfid_dashboard(request):
    ...
    if logged_in_user.role.role_name != Role.PFID:
    ...
    # POST method - Handles user registration and approvals
        ...
        if action == 'register':
            ...
            if not all([username, email, password, balai_id,phone_number, province_id, kabupaten_id]):
...
@api_view(['GET', 'POST', 'PUT', 'PATCH', 'DELETE'])
@permission_classes([IsAuthenticated])
def DPSI_dashboard(request):
    logged_in_user = request.user
    

    if logged_in_user.role.role_name != Role.DPSI:
        return Response({"detail": "Unauthorized access."}, status=status.HTTP_403_FORBIDDEN)
    # GET method - Fetches data
    if request.method == 'GET':
        balai_users_pending_approval = User.objects.filter(role__role_name=Role.BALAI, approved=False)
...
@api_view(['GET', 'POST', 'PUT', 'PATCH', 'DELETE'])
@permission_classes([IsAuthenticated])
def SPDJD_dashboard(request):
    logged_in_user = request.user
    

    if logged_in_user.role.role_name != Role.SPDJD:
        return Response({"detail": "Unauthorized access."}, status=status.HTTP_403_FORBIDDEN)
    # GET method - Fetches data
    if request.method == 'GET':
        balai_users_pending_approval = User.objects.filter(role__role_name=Role.BALAI, approved=False)
